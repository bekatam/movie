...
const MustWatch:React.FC<MWProps> = ({bebas}) => {
  ...
  return (
    <div className="mustwatch px-14 py-10 mt-24 min-h-[650px]">
        ...
        <div className="flex relative text-white justify-between">
        {results.length > 0 ? (
          results.splice(0,6).map((item: any, index: number) => {
                return (
                    <Link href={`/movie/${item.id}`} key = {index} className="flex flex-col hover:border-gray-600 w-[270px] hover:border-[1px] rounded-3xl">
                        <img src={'http://image.tmdb.org/t/p/w342' + item.poster_path} className='rounded-3xl mb-3 h-[390px]'/>
                        <button><AiOutlinePlayCircle className='text-6xl opacity-50 hover:opacity-100 text-purple-400 bg-purple-900 rounded-full absolute bottom-20 translate-x-52'/></button>
                        <p className='text-start w-full'>{item.title}</p>
